...
def create_combined_readings_chart(df, title="Lecturas acumuladas y consumo mensual"):
    """
    Creates a combined chart showing both accumulated readings and monthly consumption.
    
    Args:
        df (pd.DataFrame): DataFrame with columns 'date', 'reading', 'consumption'
    ...
    # Debug information
    print(f"[DEBUG] create_combined_readings_chart - DataFrame columns: {df.columns.tolist()}")
    print(f"[DEBUG] create_combined_readings_chart - DataFrame shape: {df.shape}")
    print(f"[DEBUG] create_combined_readings_chart - DataFrame dtypes: {df.dtypes}")
    print(f"[DEBUG] create_combined_readings_chart - Reading values: {df['reading'].tolist()}")
    print(f"[DEBUG] create_combined_readings_chart - Consumption values: {df['consumption'].tolist()}")
    
    # Ensure we have valid data
    if 'reading' not in df.columns or 'consumption' not in df.columns or 'date' not in df.columns:
    ...
    if df['reading'].isnull().all() or df['consumption'].isnull().all():
        print("[ERROR] All values are null in reading or consumption columns")
        fig = go.Figure()
        fig.add_annotation(
            text="Error en los datos: valores nulos",
            xref="paper", yref="paper",
            x=0.5, y=0.5, showarrow=False
        )
        fig.update_layout(title=title, **CHART_CONFIG)
    ...
    # Add accumulated readings as a line on primary y-axis
    fig.add_trace(
        go.Scatter(
            x=df['date'],
            y=df['reading'],
            name='Lectura acumulada',
            line=dict(color='royalblue', width=3),
            yaxis='y'
        )
    )
    
    # Add monthly consumption as bars on secondary y-axis
    fig.add_trace(
        go.Bar(
            x=df['date'],
            y=df['consumption'],
