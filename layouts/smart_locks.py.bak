...
def register_callbacks(app):
    ...
    def update_multiple_cards_for_lock(device, uuid_list, jwt_token):
        ...
        logger.info(f"Actualizando {len(uuid_list)} tarjetas para: device_id={device_id}, gateway_id={gateway_id}, asset_id={asset_id}, display_name={display_name}")
    ...
    def unassign_multiple_cards_for_lock(device, uuid_list, jwt_token):
        ...
        Returns:
            Tupla (success, message, details) con el resultado de la operación
        """
        from utils.nfc_helper import check_card_exists
        from utils.api import update_nfc_code_value, get_nfc_passwords
        
        # Usar el ID canónico del dispositivo si está disponible
        device_id = device.get('canonical_device_id') or device.get('device_id')
        asset_id = device.get('asset_id')
        gateway_id = device.get('gateway_id')
        
        logger.info(f"=== DEBUG UNASSIGN START ===")
        logger.info(f"Device ID: {device_id}")
        logger.info(f"Asset ID: {asset_id}")
        logger.info(f"Gateway ID: {gateway_id}")
        logger.info(f"UUIDs to unassign: {uuid_list}")
        logger.info(f"Device original data keys: {list(device.keys()) if device else 'None'}")
        logger.info(f"Device original sensors count: {len(device.get('sensors', []))}")
        logger.info(f"Device original sensors: {device.get('sensors', [])}")
        
        # Determinar el modo de operación
        mode = "all" if not uuid_list else "specific"
        logger.info(f"Operation mode: {mode}")
        
        try:
            # Copiar datos del dispositivo original
            updated_device = dict(device) if device else {}
            logger.info(f"Updated device initial data: {updated_device}")
            
            # Intentar obtener datos NFC actualizados desde la API
            logger.info(f"Attempting to get fresh NFC data for asset_id: {asset_id}")
            try:
                success, api_response = get_nfc_passwords(asset_id=asset_id, jwt_token=jwt_token)
                logger.info(f"API call success: {success}")
                logger.info(f"API response type: {type(api_response)}")
                logger.info(f"API response content: {api_response}")
                
                if success and api_response:
                    # El response puede ser una lista directa o un diccionario con 'devices'
                    devices = []
                    if isinstance(api_response, dict):
                        if 'devices' in api_response:
                            devices = api_response['devices']
                            logger.info(f"Found devices in API response: {len(devices)}")
                        elif 'data' in api_response:
                            data_section = api_response['data']
                            logger.info(f"Found data section type: {type(data_section)}")
                            if isinstance(data_section, dict) and 'devices' in data_section:
                                devices = data_section['devices']
                            elif isinstance(data_section, list):
                                devices = data_section
                        else:
                            logger.warning(f"API response structure unexpected: {list(api_response.keys())}")
                    elif isinstance(api_response, list):
                        devices = api_response
                        logger.info(f"API response is direct list with {len(devices)} devices")
                    
                    logger.info(f"Total devices found in API response: {len(devices)}")
                    
                    # Buscar el dispositivo específico con datos actualizados
                    found_device = False
                    for i, api_device in enumerate(devices):
                        api_device_id = str(api_device.get('device_id', ''))
                        logger.info(f"Checking device {i}: ID={api_device_id}, keys={list(api_device.keys())}")
                        logger.info(f"Device {i} sensors: {api_device.get('sensors', [])}")
                        
                        if api_device_id == str(device_id):
                            logger.info(f"Found matching device in API response: {api_device_id}")
                            # Combinar datos del dispositivo original con datos NFC actualizados
                            updated_device = dict(device)  # Copiar datos originales
                            updated_device['sensors'] = api_device.get('sensors', [])  # Usar sensores actualizados
                            logger.info(f"Updated device sensors count: {len(updated_device.get('sensors', []))}")
                            logger.info(f"Updated device sensors detail: {updated_device.get('sensors', [])}")
                            found_device = True
                            break
                    
                    if not found_device:
                        logger.warning(f"Device {device_id} not found in API response. Available device IDs: {[str(d.get('device_id', '')) for d in devices]}")
                else:
                    logger.warning(f"Failed to get API data or empty response. Success: {success}, Response: {api_response}")
                    
            except Exception as e:
                logger.error(f"Exception getting NFC data for {device_id}: {str(e)}")
                logger.error(f"Exception type: {type(e)}")
                import traceback
                logger.error(f"Full traceback: {traceback.format_exc()}")
            
            # Verificar el estado final del dispositivo actualizado
            logger.info(f"=== FINAL DEVICE STATE ===")
            logger.info(f"Updated device keys: {list(updated_device.keys())}")
            logger.info(f"Updated device has sensors: {'sensors' in updated_device}")
            logger.info(f"Updated device sensors count: {len(updated_device.get('sensors', []))}")
            logger.info(f"Updated device sensors detail: {updated_device.get('sensors', [])}")
            
            # Para el caso de dispositivos sin sensores definidos
            if "sensors" not in updated_device or not updated_device["sensors"]:
                logger.error(f"No sensors found in updated device {device_id}")
                if mode == "all":
                    return True, "No hay tarjetas para desasignar", []
                else:
                    return False, "No se encontraron sensores NFC en el dispositivo", []
            ...
            if mode == "all":
                # Desasignar TODAS las tarjetas NFC del dispositivo
                nfc_sensors = [sensor for sensor in updated_device["sensors"] if sensor.get("sensor_type") == "NFC_CODE" and sensor.get("password")]
                
                if not nfc_sensors:
                    return True, "No hay tarjetas NFC asignadas para desasignar", []
                
                for sensor in nfc_sensors:
                    slot_number = str(sensor.get("sensor_id", ""))
                    current_password = sensor.get("password", "")
                    
                    try:
                        logger.info(f"Desasignando tarjeta NFC en slot {slot_number} para dispositivo {device_id}")
                        
                        # Limpiar el valor del sensor (establecer como vacío)
                        success, response = update_nfc_code_value(
                            asset_id=asset_id,
                            device_id=device_id,
                            sensor_id=slot_number,
                        ...
                        if success:
                            logger.info(f"Tarjeta en slot {slot_number} desasignada correctamente para dispositivo {device_id}")
                            results.append({
                                "uuid": current_password,
                                "slot": slot_number,
                                "status": "success",
                                "message": f"Desasignada del slot {slot_number}"
                            })
                            ...
                            if "Token expirado" in error_message or "Token JWT expirado" in error_message or "401" in error_message:
                                logger.error(f"Error de autenticación al desasignar tarjeta en slot {slot_number} para {device_id}")
                                results.append({
                                    "uuid": current_password,
                                    "slot": slot_number,
                                    "status": "auth_error",
                                ...
                                logger.error(f"Error al desasignar tarjeta en slot {slot_number} para {device_id}: {response}")
                                results.append({
                                    "uuid": current_password,
                                    "slot": slot_number,
                                    "status": "failed",
                        ...
                        logger.error(f"Error procesando slot {slot_number} para {device_id}: {str(e)}")
                        results.append({
                            "uuid": current_password,
                            "slot": slot_number,
                            "status": "failed",
                ...
                # Desasignar tarjetas específicas
                for uuid_value in uuid_list:
                    try:
                        # Buscar la tarjeta en el dispositivo usando datos actualizados
                        card_exists, existing_slot = check_card_exists(updated_device, uuid_value)
                        
                        if not card_exists:
                            logger.info(f"La tarjeta {uuid_value} no se encontró en el dispositivo {device_id}")
                            
                            # FALLBACK: Intentar buscar en todos los slots posibles como último recurso
                            # Esto puede ayudar si los datos están desactualizados
                            fallback_attempted = False
                            fallback_success = False
                            
                            # Intentar en slots comunes donde suelen estar las tarjetas (7-20)
                            for slot_to_try in [str(i) for i in range(7, 21)]:
                                try:
                                    logger.info(f"Fallback: Intentando desasignar {uuid_value} del slot {slot_to_try} en dispositivo {device_id}")
                                    
                                    success, response = update_nfc_code_value(
                                        asset_id=asset_id,
                                        device_id=device_id,
                                        sensor_id=slot_to_try,
                                        new_value="",  # Valor vacío para desasignar
                                        jwt_token=jwt_token,
                                        gateway_id=gateway_id,
                                        is_master_card=slot_to_try == "7"
                                    )
                                    
                                    if success:
                                        logger.info(f"Fallback exitoso: Tarjeta {uuid_value} desasignada del slot {slot_to_try} en dispositivo {device_id}")
                                        results.append({
                                            "uuid": uuid_value,
                                            "slot": slot_to_try,
                                            "status": "success",
                                            "message": f"Desasignada del slot {slot_to_try} (fallback)"
                                        })
                                        successful_unassignments += 1
                                        ...
                                        # Si falla por error de autenticación, detener el fallback
                                        error_message = str(response)
                                        if "Token expirado" in error_message or "Token JWT expirado" in error_message or "401" in error_message:
                                            logger.warning(f"Fallback detenido por error de autenticación en dispositivo {device_id}")
                                            break
                            ...
                            # Si el fallback no tuvo éxito, registrar como no encontrada
                            if not fallback_success:
                                results.append({
                                    "uuid": uuid_value,
                                    "slot": None,
                                    "status": "not_found",
                                    "message": "Tarjeta no encontrada en el dispositivo" + (" (fallback intentado)" if fallback_attempted else "")
                                })
                                not_found += 1
                            continue
            ...
            if mode == "all":
                if successful_unassignments == total_processed:
                    return True, f"Todas las tarjetas ({successful_unassignments}) desasignadas correctamente", results
                elif successful_unassignments > 0:
                    return True, f"{successful_unassignments} de {total_processed} tarjetas desasignadas correctamente", results
                elif auth_errors > 0:
                    return False, f"Error de autenticación en {auth_errors} tarjetas", results
                else:
                    return False, f"No se pudo desasignar ninguna tarjeta", results
            else:
                if successful_unassignments == len(uuid_list):
                    return True, f"Todas las tarjetas especificadas ({successful_unassignments}) desasignadas correctamente", results
                elif successful_unassignments > 0:
                    return True, f"{successful_unassignments} de {len(uuid_list)} tarjetas desasignadas correctamente", results
                elif not_found == len(uuid_list):
                    return True, f"Todas las tarjetas especificadas ({not_found}) ya no estaban asignadas", results
                elif auth_errors > 0:
                    return False, f"Error de autenticación en {auth_errors} tarjetas", results
                else:
                    return False, f"No se pudo desasignar ninguna tarjeta", results
            ...
            logger.error(f"Error general desasignando tarjetas para {device_id}: {str(e)}")
            return False, f"Error: {str(e)}", []

    # Función para actualizar una cerradura individual (versión original para compatibilidad)
